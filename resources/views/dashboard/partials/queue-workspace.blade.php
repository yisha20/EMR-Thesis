...
<section class="dashboard-panel queue-workspace" aria-labelledby="queue-operations-title">
    ...
    <div class="table-responsive-shell">
        ...
        <tbody>@forelse($queueEntries as $entry)<tr>
            ...
            @else
                @if($entry->queue_type==='counter' && $entry->status==='called')
                    <form method="POST" action="{{ route('counter-services.start',$entry) }}">@csrf<button class="btn btn-sm btn-primary" data-submit-loading="Starting...">Start Service</button></form>
                @elseif($entry->queue_type==='counter' && $entry->status==='serving')
                    <a class="btn btn-sm btn-primary" href="{{ route('counter-services.show',$entry) }}">Open Service Workspace</a>
                @endif
                @foreach(['waiting'=>['called'=>'Call'],'called'=>['called'=>'Recall','missed'=>'Mark Missed']] [$entry->status] ?? [] as $state=>$label)
                ...
                @endforeach
                @if(in_array($entry->status,['waiting','called'],true) && $entry->queue_type==='counter')<a class="btn btn-sm btn-light" href="{{ route('counter-services.show',$entry) }}">Transfer to Doctor</a>@elseif(in_array($entry->status,['waiting','called'],true))<form method="POST" action="{{ route('clinic-queues.transfer',$entry) }}">@csrf<input type="hidden" name="queue_type" value="counter"><input type="hidden" name="reason" value="Transferred by clinic staff"><button class="btn btn-sm btn-light" data-confirm="Transfer {{ $entry->ticket_number }} to the counter queue?">Transfer</button></form>@endif
