...
.checkbox { border:1px solid #111; display:inline-block; font-size:8pt; font-weight:bold; height:11px; line-height:10px; margin-right:4px; text-align:center; vertical-align:middle; width:11px; }
...
@php
    ...
    $selectedFamily=$assessment->familyHistories->keyBy('condition');
    $familyConditions=\App\Http\Controllers\HealthAssessmentController::FAMILY_CONDITIONS;
...
<div class="section-title">B. Past Medical History</div>
<table class="history-grid">
@foreach(collect($medicalConditions)->chunk(3) as $row)<tr>@foreach($row as $condition)@php($record=$selectedMedical->get($condition) ?: ($condition==='Other'?$assessment->medicalHistories->first(function($x){return strpos($x->condition,'Other:')===0;}):null))<td><span class="checkbox">{{$record?'✓':''}}</span>{{$condition}}@if($record&&($record->notes||$record->medication))<br><small>{{$record->notes}} {{$record->medication?'Medication: '.$record->medication:''}}</small>@endif</td>@endforeach @for($i=$row->count();$i<3;$i++)<td></td>@endfor</tr>@endforeach
</table>

<div class="section-title">C. Women's Health (when applicable)</div>
...
<div class="section-title">D. Family History</div>
<table>
<tr><td colspan="3"><span class="label">Close relative (parent, grandparent, or sibling) diagnosed with</span></td></tr>
@foreach(collect($familyConditions)->chunk(3) as $row)<tr>@foreach($row as $condition)@php($item=$selectedFamily->get($condition))<td><span class="checkbox">{{$item?'✓':''}}</span>{{$condition}}@if($item&&$item->details)<br><small>{{$item->details}}</small>@endif</td>@endforeach @for($i=$row->count();$i<3;$i++)<td></td>@endfor</tr>@endforeach
</table>

<div class="section-title">E. Social History and Current Medications</div>
<table>
<tr><td><span class="label">Do you smoke?</span>{{['Never'=>'No','Current smoker'=>'Yes','Former smoker'=>'Former smoker'][data_get($assessment,'social_history.smoking_status')] ?? 'Not reported'}}</td><td><span class="label">If yes, packs per day</span>{{data_get($assessment,'social_history.smoking_packs') ?: 'Not applicable'}}</td></tr>
<tr><td><span class="label">Do you drink alcohol?</span>{{data_get($assessment,'social_history.drinks_alcohol','Not reported')}}@if(data_get($assessment,'social_history.alcohol_type')) — {{data_get($assessment,'social_history.alcohol_type')}}@endif</td><td><span class="label">How frequent?</span>{{data_get($assessment,'social_history.alcohol_frequency') ?: 'Not applicable'}}</td></tr>
<tr><td><span class="label">Do you take medications at present?</span>{{data_get($assessment,'social_history.takes_medications') ?: ($assessment->medications->isNotEmpty() ? 'Yes' : 'No')}}</td><td><span class="label">Current medications</span>{{$assessment->medications->pluck('medication')->implode(', ') ?: 'None reported'}}</td></tr>
</table>

<div class="section-title">F. Registered Dependents</div>
...
<div class="section-title">G. Physical Examination - Clinic Staff Only</div>
...
@foreach($systems as $system)@php($exam=$physical[$system]??$physical[\Illuminate\Support\Str::slug($system,'_')]??[])<tr><td>{{$system}}</td>@foreach(['normal','abnormal','not_examined'] as $state)<td class="center"><span class="checkbox">{{data_get($exam,'status')===$state?'✓':''}}</span></td>@endforeach<td>{{$exam['remarks']??''}}</td></tr>@endforeach
</tbody></table>

<div class="section-title">H. Vital Signs - Clinic Staff Only</div>
...
<div class="section-title">I. Nursing Interventions</div>
...
<div class="section-title">J. Assessment and Recommendations - Physician Only</div>
@if(empty($clinical))<div class="staff-note">For authorized physician completion</div>@endif
<table><tr><td><span class="checkbox">{{data_get($clinical,'fitness_status')==='physically_fit'?'✓':''}}</span>Physically Fit</td><td><span class="checkbox">{{data_get($clinical,'fitness_status')==='not_physically_fit'?'✓':''}}</span>Not Physically Fit</td><td><span class="checkbox">{{data_get($clinical,'fitness_status')==='fit_with_restrictions'?'✓':''}}</span>Fit with Restrictions</td><td><span class="checkbox">{{data_get($clinical,'fitness_status')==='further_evaluation'?'✓':''}}</span>Further Evaluation Required</td></tr>
